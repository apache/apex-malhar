...
public class YahooFinanceApplication implements ApplicationFactory
{
  ...
  {
    AverageKeyVal<String> oper = dag.addOperator(name, AverageKeyVal.class);
    //oper.setType(Double.class);
    return oper;
  }
  ...
  public DAG getApplication(Configuration conf)
  {
    ...
    if (windowTest) {
      ...
      dag.addStream("volume_pvConsolidator", dailyVolume.sum, pvConsolidator.data2).setInline(false);
      dag.addStream("pvConsolidator_console", pvConsolidator.out, windowedConsole.input).setInline(false);
    }
    else if (timeTest) {
      ...
      dag.addStream("priceAverage_priceSma", priceAvg.doubleAverage, priceSMA.data).setInline(true);
      ...
      dag.addStream("volumeAverage_volumeSma", volumeAvg.intAverage, volumeSMA.data).setInline(true);
