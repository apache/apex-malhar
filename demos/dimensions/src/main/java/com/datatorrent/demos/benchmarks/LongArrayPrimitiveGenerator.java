 ...
 */
public class LongArrayPrimitiveGenerator implements InputOperator
{
  @Min(1)
  ...
  public LongArrayPrimitiveGenerator()
  {
  }
  ...
  {
    for(;
        arrayCounter < getNumArrays();
        arrayCounter++) {
      long[] outputArray = new long[getArrayLength()];//(long) arrayCounter;

      for(int arrayIndex = 0;
          arrayIndex < getArrayLength();
          arrayIndex++) {
