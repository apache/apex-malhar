...
public class JdbcPojoPollableOpeartorTest extends JdbcOperatorTest
{
  ...
  public void beforeTest()
  {
    ...
    when(mockschedular.scheduleWithFixedDelay(any(Runnable.class), anyLong(), anyLong(), any(TimeUnit.class)))
  ...
  public void testDBPoller() throws InterruptedException
  {
    ...
    for (com.datatorrent.api.Partitioner.Partition<AbstractJdbcPollInputOperator<Object>> partition : newPartitions) {
      ...
      parition.setScheduledExecutorService(mockschedular);
  ...
  public void testRecovery() throws IOException
  {
    ...
    inputOperator.setBatchSize(100);
    inputOperator.setKey("id");
    inputOperator.lastEmittedRecord = 0; //setting as not calling partition logic
    inputOperator.setRangeQueryPair(new KeyValPair<Integer, Integer>(0, 8));

    inputOperator.outputPort.setup(tpc);
    inputOperator.setScheduledExecutorService(mockschedular);
    ...
    verify(mockschedular, times(0)).scheduleAtFixedRate(any(Runnable.class), anyLong(), anyLong(), any(TimeUnit.class));
    inputOperator.emitTuples();
    inputOperator.endWindow();
    inputOperator.beginWindow(1);
    verify(mockschedular, times(1)).scheduleAtFixedRate(any(Runnable.class), anyLong(), anyLong(), any(TimeUnit.class));

    System.out.println("Tuples count: " + sink.collectedTuples.size());
