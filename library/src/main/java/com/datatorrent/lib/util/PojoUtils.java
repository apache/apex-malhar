...
public class PojoUtils
{
  ...
  private static final String DEFAULT_POJO_NAME = "object";
  ...
  public static String getSingleFieldGetterExpression(Class<?> clazz, final String fieldExpression)
  {
    try {
      ...
      logger.debug("Class {} does not have field {}. Proceeding to locate a getter method.", clazz.getName(), fieldExpression, ex);
    }
    ...
    try {
      ...
      logger.debug("Class {} does not have method {}. Proceeding to locate another getter method.",
              clazz.getName(), methodName, ex);
    }
    ...
    try {
      ...
      logger.debug("Class {} does not have method {}. Proceeding with the original expression {}.",
              clazz.getName(), methodName, fieldExpression, ex);
    }
  ...
  private static String getSingleFieldSetterExpression(final Class<?> pojoClass, final String fieldExpression, final Class<?> exprClass)
  {
    ...
    try {
      ...
      if (ClassUtils.isAssignable(exprClass, type)) {
        ...
        logger.debug("Class {} does not have a field that {} may be assigned to. Proceeding to locate a setter method.",
                pojoClass.getName(), exprClass.getName());
      }
    } catch (NoSuchFieldException ex) {
      logger.debug("Class {} does not have field {}. Proceeding to locate a setter method.", pojoClass.getName(), fieldExpression, ex);
    }
    ...
    if (bestMatchMethod == null) { // We did not find the exact match, use candidates to find the match
      if (candidates.size() == 0) {
        logger.debug("Class {} does not have suitable setter method {}. Returning original expression {}.",
                pojoClass.getName(), setMethodName, fieldExpression);
