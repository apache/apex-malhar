...
This document describes the best practices to follow when developing operators or other application components such as partitoners, stream codecs etc.

## Operators

These are general guidelines for all operators and the next sections talk about special considerations for input and output operators.
...
* Follow the conventions of the life cycle methods
	...
	* Perform periodic tasks based on processing time in application windows
...
* If an operator needs to do some work when it is not receiving any input, it should implement **IdleTimeHandler** interface. This interface contains **handleIdleTime** method which will be called whenever the platform isn’t doing anything else and the operator can do the work in this method. If for any reason the operator is not going to do any work in this method, it should sleep for a small amount of time such as that specified by the **SPIN_MILLIS** attribute otherwise it will cause a busy wait.
...
* All variables in the operator not marked transient and final are saved so any variables in the operator that are not part of the state should be marked transient. Specifically any variables like connection objects, i/o streams, ports are transient, because they need to be setup again on failure recovery.
* If the operator does not keep any state between windows, mark it with the **@Stateless** annotation. This results in efficiencies during checkpointing and recovery. The operator will not be checkpointed and always restored to the initial state
...
* If the operator state is going to be large, checkpointing the entire state each time becomes unviable. Furthermore, the amount of state to keep could be larger than the amount of physical memory available. In those cases operator developers should use more efficient ways of maintaining state such as using **ManagedState** utility provided by the platform which uses a combination of in memory and disk cache to efficiently store and retrieve data in a performant and fault tolerant way. There are operators in the platform that use *ManagedState* and can be used as a reference on how to use this utility such as Dedup or Join operators.

## Input Operators

Input operators have additional requirements

* The **emitTuples** method is called by platform giving it an opportunity to emit some data. There are some guidelines on how to implement this method that are important
	* This should not be a blocking method and should return in a reasonable time that is less than the streaming window size (which is 500ms by default).
	...
	* The method can emit more than one data tuples.
...
* File output operator that writes data to files keeps track of the file lengths in the state. These lengths are checkpointed and restored on failure recovery. On restart, the operator truncates the file to the offset equal to the recovered length. This makes the data in the file same as it was at the time of checkpoint before the failure. The operator now writes the replayed data from the checkpoint in regular fashion as any other data. This ensures no data is lost or duplicated in the file.
...
Apex comes with some built-in partitioning utilities that can be used in certain scenarios.

* **StatelessPartitioner** in Malhar provides a default partitioner, which can be used in certain conditions. If the operator satisfies these conditions, no special partitioning code is needed . These are
No dynamic partitioning is needed, see next point about dynamic partitioning. 
There is no distinct initial state for the partitions, i.e., all partitions start with the same initial state submitted during application launch. Typically input or output operators will not fall into this category, although there are exceptions and mainly it is operators in the middle of the DAG that are in this category.
When used with non-input operators, only the data for the first declared input port is distributed among the different partitions. All other input ports are treated as broadcast and all partitions receive all the data for that port.
* **StatelessThroughputBasedPartitioner** in Malhar provides a dynamic partitioner based on throughput thresholds. Similarly **StatelessLatencyBasedPartitioner** provides a latency based dynamic partitioner in RTS. If these partitioners can be used, then separate partitioning related code is not needed. The conditions under which these can be used are
	...
	* There is no state change in the operator across windows i.e., operator is stateless.
