...
public class WindowedOperatorTest
{
  ...
  private WindowedOperatorImpl<Long, MutableLong, Long> createDefaultWindowedOperator()
  {
    ...
    if (useSpillable) {
      ...
      // TODO: We don't yet support Spillable data structures for window state storage.
  ...
  private KeyedWindowedOperatorImpl<String, Long, MutableLong, Long> createDefaultKeyedWindowedOperator(boolean forSession)
  {
    ...
    if (useSpillable) {
      ...
      if (forSession) {
        ...
        SpillableSessionWindowedStorage<String, Long> sws2 = new SpillableSessionWindowedStorage<>();
        sws2.setSpillableComplexComponent(sccImpl);
        keyedRetractionStorage = sws2;
      } else {
