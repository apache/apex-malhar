 ...
 * @since 1.0.2
 */
public class StatelessLatencyBasedPartitioner<T extends Operator> implements StatsListener, Partitioner<T>, Serializable
{
  private static final Logger logger = LoggerFactory.getLogger(StatelessLatencyBasedPartitioner.class);
  ...
  public Response processStats(BatchedOperatorStats stats)
  {
    ...
    if (stats.getLatencyMA() < minimumLatency || stats.getLatencyMA() > maximumLatency) {
  ...
  public Collection<Partition<T>> definePartitions(Collection<Partition<T>> partitions, PartitioningContext context)
  {
    if (partitionedInstanceStatus == null || partitionedInstanceStatus.isEmpty()) {
      ...
      for (Partition<T> p : partitions) {
        ...
        else if (stats.getLatencyMA() < minimumLatency) {
          load = -1;
        }
        if (load < 0) {
