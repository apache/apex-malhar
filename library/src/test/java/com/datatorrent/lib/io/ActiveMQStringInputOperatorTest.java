...
import com.datatorrent.lib.testbench.ActiveMQMessageGenerator;
...
/**
 * Functional test for {@link com.datatorrent.lib.io.ActiveMQStringInputOperator}.
 */
public class ActiveMQStringInputOperatorTest
{
	private static final Logger logger = LoggerFactory
			.getLogger(ActiveMQStringInputOperatorTest.class);
	...
	private void startActiveMQService() throws Exception
	{
		...
		broker.getPersistenceAdapter().setDirectory(
				new File("target/test-data/activemq-data/" + broker.getBrokerName()
						+ "/KahaDB"));
	...
	{
		public final transient CollectorInputPort<T> inputPort = new CollectorInputPort<T>(
				"myInput", this);
	}
	...
	{
		public final transient CollectorInputPort<T1> inputPort1 = new CollectorInputPort<T1>(
				"myInput1", this);
		public final transient CollectorInputPort<T2> inputPort2 = new CollectorInputPort<T2>(
				"myInput2", this);
	}
	...
	/**
	 * Test AbstractActiveMQSinglePortInputOperator (i.e. an input adapter for
	 * ActiveMQ, aka consumer). This module receives data from an outside test
	 * generator through ActiveMQ message bus and feed that data into Malhar
	 * streaming platform.
	 * 
	 * [Generate message and send that to ActiveMQ message bus] ==> [Receive that
	 * message through ActiveMQ input adapter(i.e. consumer) and send using
	...
	public void testActiveMQInputOperator() throws Exception
	{
		...
		ActiveMQStringInputOperator node = dag.addOperator("AMQ message consumer",
				ActiveMQStringInputOperator.class);
		...
		final CollectorModule<String> collector = dag.addOperator(
				"TestMessageCollector", new CollectorModule<String>());

		// Connect ports
		dag.addStream("AMQ message", node.outputPort, collector.inputPort)
				.setInline(true);
		...
		Assert.assertEquals("Tuple count", 7,
				collections.get(collector.inputPort.id).size());
	}
	...
	public void testActiveMQInputOperator2() throws Exception
	{
		...
		// Check results
		Assert.assertEquals("Collections size", 1, collections.size());
		Assert.assertEquals("Tuple count", 20,
				collections.get(collector.inputPort.id).size());
	}

	public static class ActiveMQMultiPortInputOperator extends
	...
	public void testActiveMQMultiPortInputOperator() throws Exception
	{
		...
		LogicalPlan dag = new LogicalPlan();
		// Create ActiveMQStringSinglePortInputOperator
		ActiveMQMultiPortInputOperator node = dag.addOperator(
				"AMQ message consumer", ActiveMQMultiPortInputOperator.class);
		// Set configuration parameters for ActiveMQ
		node.setUser("");
		node.setPassword("");
		node.setUrl("tcp://localhost:61617");
		...
		// Create Test tuple collector
		final CollectorModule2<String, Integer> collector = dag.addOperator(
				"TestMessageCollector", new CollectorModule2<String, Integer>());

		// Connect ports
		dag.addStream("AMQ message", node.outputPort1, collector.inputPort1)
				.setInline(true);
		dag.addStream("AMQ message2", node.outputPort2, collector.inputPort2)
				.setInline(true);
		...
		Assert.assertEquals("Tuple count", 20,
				collections.get(collector.inputPort1.id).size());
		Assert.assertEquals("Tuple count", 0,
				collections.get(collector.inputPort2.id).size());
	}
}
