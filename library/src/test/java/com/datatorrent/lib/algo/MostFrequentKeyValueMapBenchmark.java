...
/**
 *
 * Performance tests for {@link com.datatorrent.lib.algo.MostFrequentKeyValueMap}<p>
 *
 */
public class MostFrequentKeyValueMapBenchmark
{
  private static Logger log = LoggerFactory.getLogger(MostFrequentKeyValueMapBenchmark.class);
    ...
    for (Object o: matchSink.collectedTuples) {
      ...
      for (Map.Entry<String, HashMap<Integer, Integer>> e: omap.entrySet()) {
        ...
        if (key.equals("a")) {
          ...
          Assert.assertEquals("Key \"a\" has value ", numTuples * 5, vcount);
        }
          ...
          Assert.assertEquals("Key \"a\" has value ", numTuples * 10, vcount);
        }
      }
    }
