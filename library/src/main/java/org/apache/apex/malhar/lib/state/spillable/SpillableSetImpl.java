...
public class SpillableSetImpl<T> implements Spillable.SpillableSet<T>, Spillable.SpillableComponent
{
  ...
  public static class ListNodeSerde<T> implements Serde<ListNode<T>>
  {
    ...
    @Override
    public void serialize(ListNode<T> object, SerializationBuffer buffer)
    {
      buffer.write(object.valid ? trueByte : falseByte);
      serde.serialize(object.next, buffer);
    }

    @Override
    public ListNode<T> deserialize(byte[] buffer, MutableInt offset, int length)
    {
      ListNode<T> result = new ListNode<>();
      result.valid = buffer[offset.intValue()] != 0;
      offset.add(1);
      result.next = serde.deserialize(buffer, offset, length - 1);
      return result;
    }
  }
